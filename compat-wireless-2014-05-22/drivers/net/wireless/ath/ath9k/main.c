...
static void ath_node_detach(struct ath_softc *sc, struct ieee80211_sta *sta)
{
	...
	printk("Daekyeong: ath_node_detach()\n");
...
void ath9k_tasklet(unsigned long data)
{
	...
	if (status & ATH9K_INT_FATAL) {
		printk("Daekyeong: status & ATH9K_INT_FATAL\n");
	...
	if (status & ATH9K_INT_GTT) {
		printk("Daekyeong: status & ATH9K_INT_GTT\n");
		sc->gtt_cnt++;
	...
	if ((status & ATH9K_INT_TSFOOR) && sc->ps_enabled) {
		printk("Daekyeong: status & ATH9K_INT_TSFOOR\n");
		/*
	...
	if (status & rxmask) {
		// this makes too many dmesgs.
		//printk("Daekyeong: status & rxmask\n");
	...
	if (status & ATH9K_INT_TX) {
		if (ah->caps.hw_caps & ATH9K_HW_CAP_EDMA) {
			// this makes too many dmesgs.
			//printk("Daekyeong: status & ATH9K_INT_TX EDMA\n");
			/*
	...
	if (status & ATH9K_INT_GENTIMER){
		printk("Daekyeong: status & ATH9K_INT_GENTIMER\n");
...
#define SCHED_INTR (				\
	...
	if (status & ATH9K_INT_SWBA){
		// this makes too many dmesgs.
		//printk("Daekyeong: status & ATH9K_INT_SWBA\n");
		tasklet_schedule(&sc->bcon_tasklet);
	}

	if (status & ATH9K_INT_TXURN){
		printk("Daekyeong: status & ATH9K_INT_TXURN\n");
		ath9k_hw_updatetxtriglevel(ah, true);
	}

	if (status & ATH9K_INT_RXEOL) {
		printk("Daekyeong: status & ATH9K_INT_RXEOL\n");
		ah->imask &= ~(ATH9K_INT_RXEOL | ATH9K_INT_RXORN);
		ath9k_hw_set_interrupts(ah);
	}

	if (!(ah->caps.hw_caps & ATH9K_HW_CAP_AUTOSLEEP))
		printk("Daekyeong: status & ATH9K_HW_CAP_AUTOSLEEP\n");
